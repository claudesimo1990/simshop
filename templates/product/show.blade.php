    ...
    <div class="bg-white">
        <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
            <div class="lg:grid lg:grid-cols-2 lg:items-start lg:gap-x-8">
                <!-- Image gallery -->
                <div class="flex flex-col-reverse" x-data="{ total: {{ $product->images->count() }}, current: {{ $product->image->id  }}, open: false }">
                    <!-- Image selector -->
                    <div class="mx-auto mt-6 hidden w-full max-w-2xl sm:block lg:max-w-none">
                        <div class="grid grid-cols-4 gap-6" aria-orientation="horizontal" role="tablist">
                            @foreach($product->images as $image)
                                <button @click="current = {{ $image->id }}, open = true" id="tabs-{{ $image->id }}-tab-{{ $image->id }}" class="relative flex h-24 cursor-pointer items-center justify-center rounded-md bg-white text-sm font-medium uppercase text-gray-900 hover:bg-gray-50 focus:outline-none focus:ring focus:ring-opacity-50 focus:ring-offset-4" aria-controls="tabs-{{ $image->id }}-panel-{{ $image->id }}" role="tab" type="button">
                                    <span class="sr-only">Angled view</span>
                                    <span class="absolute inset-0 overflow-hidden rounded-md">
                                        {{ $image('thumb')->attributes(['alt' => 'Angled front view with bag zipped and handles upright.', 'class' => 'h-full w-full object-cover object-center sm:rounded-lg']) }}
                                    </span>
                    ...
                    <div class="aspect-h-1 aspect-w-1 w-full">
                        @foreach($product->images as $img)
                            <div x-cloak x-show="current == {{ $img->id }}" id="tabs-{{ $img->id }}-panel-{{ $img->id }}" aria-labelledby="tabs-{{ $img->id }}-tab-{{ $img->id }}" role="tabpanel" tabindex="{{ $img->id }}" x-transition>
                                {{ $img->img()->attributes(['alt' => 'Angled front view with bag zipped and handles upright.', 'class' => 'h-full w-full object-cover object-center sm:rounded-lg']) }}
                            </div>
                        @endforeach
                        <!-- More images... -->
                    </div>
                ...
                <div class="mt-10 px-4 sm:mt-16 sm:px-0 lg:mt-0">
                    ...
                    <div class="mt-3">
                        ...
                        <p class="text-3xl tracking-tight text-gray-900">{{ $product->price }}&euro;</p>
                    </div>
                    ...
                    <form class="mt-6" action="{{ route('cart.product.add', $product) }}" method="POST" enctype="multipart/form-data">
                        <div>
                        ...
                        <div class="mt-10 flex">
                            ...
                            <input type="submit" class="flex max-w-xs flex-1 items-center justify-center rounded-md border border-transparent bg-indigo-600 px-8 py-3 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50 sm:w-full" value="Add to bag"/>
                    ...
                    <section aria-labelledby="details-heading" class="mt-12">
                        ...
                        <div class="divide-y divide-gray-200 border-t">
                            <div x-data="{showFeature: false, collapse() {this.showFeature = !this.showFeature}}">
                                <h3>
                                    <!-- Expand/collapse question button -->
                                    <button type="button" @click="collapse()" class="group relative flex w-full items-center justify-between py-6 text-left" aria-controls="disclosure-1" aria-expanded="false">
                                ...
                                <div class="prose prose-sm pb-6" id="disclosure-1" x-cloak x-show="showFeature" x-transition>
                                    <ul role="list">
